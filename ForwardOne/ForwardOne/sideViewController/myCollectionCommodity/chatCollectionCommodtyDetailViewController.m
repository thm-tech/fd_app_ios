//
//  chatCollectionCommodtyDetailViewController.m
//  ForwardOne
//
...
@interface chatCollectionCommodtyDetailViewController () <UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,UIActionSheetDelegate,YBShopDetailChangeGroupNameDelegate>
{
...
@implementation chatCollectionCommodtyDetailViewController
...
#pragma mark-(下载商品所在商店的信息)
-(void)downLoadGoodsOfShopInfo
{
...
-(void)createUINav
{
    ...
    //导航栏右按钮
    UIButton *rightButton = [ZCControl createButtonWithFrame:CGRectMake(0, 0, 25, 20) ImageName:@"visitShop" Target:self Action:@selector(rightBtn:) Title:nil];
...
//    [sharedSheet showInView:self.view];
    ...
    if(picArray.count != 0)
    {
        vc.shopPic = picArray[0];
    }
    
    vc.shopNamestrting = _shopInformationDict[@"name"];
...
-(void)YBShopDetailChangeGroupGname:(NSString *)gname andGroupName:(NSString *)groupName
{
    ...
    [self.YB_delegate YBchatCollectionDetailChatChangeGnameWithGname:gname andGroupName:groupName];
}
