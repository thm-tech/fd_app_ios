...
//对消息内容进行处理（当为图片的时候点击放大，当为文字的时候点击复制，当为语音的时候点击播放或者再点击停止播放）图片的选择
...
//        }
        if([self.delegate respondsToSelector:@selector(cellContentDidClick:image:)])
        {
            [self.delegate cellContentDidClick:self image:self.messageFrame.message.picture];
        }
...
//                                 placeholderImage:[UIImage imageNamed:@"headImage.jpeg"]];
    ...
    //使用SDWebImage  
    [self.btnHeadImage sd_setImageWithURL:[NSURL URLWithString:message.strIcon] forState:UIControlStateNormal];
    ...
    UIImage *normal;
    if (message.from == UUMessageFromMe) {
        normal = [UIImage imageNamed:@"聊天气泡绿色"];
        normal = [normal resizableImageWithCapInsets:UIEdgeInsetsMake(35, 10, 10, 22)];
    }
    else{
        normal = [UIImage imageNamed:@"聊天气泡白色"];
    ...
    switch (message.type) {
        ...
        case UUMessageTypePicture:
        {
            ...
            [self.btnContent.backImageView sd_setImageWithURL:[NSURL URLWithString:(NSString *)message.picture]];
            ...
            if(message.from == 0)
            {
            if([message.mmxType isEqualToString:@"png"])
            {
                self.btnContent.iconImageView.hidden = YES;
            }
            else if ([message.mmxType isEqualToString:@"mmx/goods"])
            {
                self.btnContent.sendTitleLabel.text = [NSString stringWithFormat:@"%@",message.title];
                self.btnContent.mmxTypeImageView.image = [UIImage imageNamed:@"商品"];
            }
            else if ([message.mmxType isEqualToString:@"mmx/act"])
            {
                self.btnContent.sendTitleLabel.text = [NSString stringWithFormat:@"%@",message.title];
                self.btnContent.mmxTypeImageView.image = [UIImage imageNamed:@"活动"];
            }
            else if ([message.mmxType isEqualToString:@"mmx/shop"])
            {
                self.btnContent.sendTitleLabel.text = [NSString stringWithFormat:@"%@",message.title];
                self.btnContent.mmxTypeImageView.image = [UIImage imageNamed:@"商店"];
            }
            }
            else
            {
                ...
                {
                    self.btnContent.sendTitleLabel.text = [NSString stringWithFormat:@"%@",message.title];
                    self.btnContent.mmxTypeImageView2.image = [UIImage imageNamed:@"商品"];
                }
                else if ([message.mmxType isEqualToString:@"mmx/act"])
                {
                    self.btnContent.sendTitleLabel.text = [NSString stringWithFormat:@"%@",message.title];
                    self.btnContent.mmxTypeImageView2.image = [UIImage imageNamed:@"活动"];
                }
                else if ([message.mmxType isEqualToString:@"mmx/shop"])
                {
                    self.btnContent.sendTitleLabel.text = [NSString stringWithFormat:@"%@",message.title];
                    self.btnContent.mmxTypeImageView2.image = [UIImage imageNamed:@"商店"];
                }
        ...
        case UUMessageTypeVoice:
        {
            ...
            self.btnContent.second.text = [NSString stringWithFormat:@"%@ Voice",message.strVoiceTime];
            //声音的二进制数据
            //songData = message.voice;
            
            NSString *voiceString =(NSString *)message.voice;
            
            NSLog(@"声音的URL = %@",voiceString);
            
            NSRange range = [voiceString rangeOfString:@".com/"];
            NSString *voiceNameSring = [voiceString substringFromIndex:range.location+5];
            NSString *path = [NSString stringWithFormat:@"%@/Documents/%@",NSHomeDirectory(),voiceNameSring];
            NSData *firstVoiceData = [[NSData alloc]initWithContentsOfURL:[NSURL fileURLWithPath:path]];
            if(firstVoiceData != nil)
                                      {
                                          songData  = firstVoiceData;
                                      }
                                      else
                                      {
            ...
            //下载使用的类 - AFURLSessionManager（）
                        ...
                        NSURLSessionDownloadTask *downloadTask = [manager downloadTaskWithRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:voiceString]] progress:&progress destination:^NSURL *(NSURL *targetPath, NSURLResponse *response) {
                            ...
                            if(error == nil)
                            {
                                ...
                                NSData *VoiceData = [[NSData alloc]initWithContentsOfURL:filePath];
                                songData = VoiceData;
                                
                                
                                NSLog(@"hahahhahahahahahhahahahahha");
                               // NSLog(@"声音的数据声音的数据%@",VoiceData);
                            }
                            else
                            {
